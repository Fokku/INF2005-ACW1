...
Demo dates: Week 5 (in your lab classes: Tues and Thurs, each team’s demo schedule will be provided later). Plan for a team demo not more than 25 mins, with all members involved. Upload your demo plan (sequence of what to show, who is showing) to xSite with all members duly signed Declaration of Originality form one day before the demo evaluation.
...
- 6. Design, explain and secure the start location (other than top-left corner) used to embed and extract the payload in each cover object.
...
- Required cover objects: image and audio. The image implementation may use PNG for the basic version. The audio implementation may use WAV/PCM for the basic version. The GUI able to play(execute) payload / display both cover and stego objects for comparison before and after encoding and decoding


- Required security components: steganographic payload, cryptographic hash and digital signature verification.

- Required submission format: with supporting source code (you cannot use online steganography portals), sample files, test evidence, README, declaration of originality form, and agreed team contribution/distribution statement.

- Required 25 mins demo duration: All members must have allocated time to talk/demonstrate about his/her part/s. Zero marks for absence.
...
- cover object and payload capacity check (is payload size larger than cover object size?).

- showing the stego object being sent from party A to party B (eg, via email), party B downloads to his/her folder and extracts the hidden message with proof of the message integrity, signature verification.

- various payload sizes (message lengths) - use one of the Learning Objectives as short message, the Project Overview paragraph as large message and a relevant custom payload your team decide that can protect the hidden message’s confidentiality and integrity.

- selectable LSBs from bits 1 to 8 of the cover object. Selection of number of LSBs to be implemented as part of GUI.
...
| FR5: Image steganographic embedding | The system must embed the payload and/or signature into the image using LSB replacement steganographic method. |
| FR6: Audio steganographic embedding | The system must embed the payload and/or signature into the audio cover object using LSB replacement steganographic method. |
...
Email to khengleong.tan@singaporetech.edu.sg with subject title “ACW1 used in genAI query” and provide device/source [URL 🔗](mailto:khengleong.tan@singaporetech.edu.sg)
...
Team may design their own implementation, but the following workflow is suggested for both image and audio cover objects:
...
The following are optional extensions and should not be attempted at the expense of the mandatory image and audio requirements. Anyone of these below can be considered as the innovation component:
...
| Steganalysis | Using known algorithms or methodologies, analyse a stego object and convincing infer the cover object is tampered or shows signs of hidden payload. |
...
| Demo plan | Planned demo for a duration of =< 25 mins (including members’ ‘air’ time) |
...
Each team must submit a signed declaration of originality and an agreed contribution/distribution statement together with the project files. Replace Px-x with your team number, eg P1-4.
...
- substitute, for own learning and thinking. You can use them to explore different ideas, generate drafts, or check your work, but NOT fo copy/ submit the generated content as your own Use generative Al tools as a supplement, not a your

- Acknowledge and cite the generative Al tools you your assignment. You should also used in explain how you used the generated content and how evaluated its quality and you relevance.

- Do not use generative Al tools to produce content that violates the academic integrity policy.

- Do not use generative Al tools fo plagiarize or fabricate content from other sources

- X Do not rely on generative Al tools to produce content that is accurate, relevant, and original

- xX It lacks critical thinking, creativity, and originality that are essential for academic work. Therefore, you should always verify, edit, and cite the generated content before using it in your assignment.
